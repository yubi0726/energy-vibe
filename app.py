...
try:
    if uploaded_file is not None:
        ...
        if not required_cols.issubset(set(user_df.columns)):
            st.sidebar.warning("⚠️ 업로드 파일 양식이 다릅니다. 기본 샘플 데이터로 대체합니다.")
    ...
    kepco_cost = calculate_kepco_cost(df_filtered, base_rate=base_rate, rates=rates_dict)
    ...
    with tab2:
        st.subheader("💰 KEPCO 산업용 계절·시간대별 요금 상세 분석")
        ...
        with col_left:
            st.markdown("#### 📊 부하별 요금 명세표")
            table_data = {
                "부하 구분": ["경부하", "중간부하", "최대부하", "기본요금 (피크 기준)"],
                "적용 단가": ["65.2 원/kWh", "109.0 원/kWh", "191.1 원/kWh", f"{base_rate:,.0f} 원/kW"],
                "사용량/용량": [
                    f"{cat_usage.get('off_peak', 0.0):,.1f} kWh",
                    f"{cat_usage.get('mid_peak', 0.0):,.1f} kWh",
                    f"{cat_usage.get('on_peak', 0.0):,.1f} kWh",
                    f"{peak_kw:.1f} kW"
                ],
                "추정 금액": [
                    f"{int(cat_usage.get('off_peak', 0.0) * rates_dict['off_peak']):,} 원",
                    f"{int(cat_usage.get('mid_peak', 0.0) * rates_dict['mid_peak']):,} 원",
                    f"{int(cat_usage.get('on_peak', 0.0) * rates_dict['on_peak']):,} 원",
                    f"{int(kepco_cost['base_cost']):,} 원"
                ]
            }
    ...
    with tab4:
        st.markdown("""
        ### 사용 도움말
        1. **AI 에너지 운영 진단:** 피크 발생 시각을 분석하여 최적의 부하 이전 가이드를 제공합니다.
        2. **부하별 요금 명세표:** 경부하, 중간부하, 최대부하별 단가와 사용 요금을 한눈에 볼 수 있습니다.
        """)
